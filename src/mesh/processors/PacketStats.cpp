...
ProcessResult PacketStats::processPacket(const PacketEvent& event, ProcessingContext& ctx) {
    totalPackets++;
    ...
    if (event.rssi > maxRssi) maxRssi = event.rssi;
    if (event.rssi < minRssi) minRssi = event.rssi;
    rssiSum += event.rssi;
