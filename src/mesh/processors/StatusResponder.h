...
class StatusResponder : public MeshCore::IPacketProcessor {
public:
  StatusResponder() : lastPayloadHash(0), lastPayloadTime(0), lastResponseTime(0), 
  ...
  const char *getName() const override { return "StatusResponder"; }
...
private:
  ...
  static constexpr uint32_t DEDUP_TIMEOUT_MS = 60000; // 60 seconds for !status deduplication
