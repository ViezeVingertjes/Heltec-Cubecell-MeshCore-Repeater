...
ProcessResult TraceHandler::processPacket(const PacketEvent& event, ProcessingContext& ctx) {
    ...
    LOG_INFO_FMT("TRACE packet: tag=0x%08lX, auth=0x%08lX, flags=0x%02X, path_len=%d, snr_len=%d, our_hash=0x%02X",
                 traceTag, authCode, flags, pathHashesLen, event.packet.pathLength, ourHash);
    ...
    if (!isNodeHashMatch(nextHopHash)) {
        LOG_INFO_FMT("TRACE not for us: next_hop=0x%02X, our_hash=0x%02X", nextHopHash, ourHash);
        return ProcessResult::DROP;
    }
...
bool TraceHandler::shouldForwardTrace(const DecodedPacket& packet, const ProcessingContext& ctx) const {
    if (ctx.isDuplicate) {
...
bool TraceHandler::appendSnrAndForward(DecodedPacket& packet, int8_t snr) {
    ...
    int32_t snrDb = snr / 4;
    LOG_INFO_FMT("Appended SNR=%d dB to TRACE path (new path_len=%d)", snrDb, packet.pathLength);
...
void TraceHandler::handleTraceComplete(const DecodedPacket& packet) {
    ...
    LOG_INFO_FMT("TRACE complete! tag=0x%08lX, total hops=%d", traceTag, packet.pathLength);
    ...
    for (uint8_t i = 0; i < packet.pathLength; i++) {
        ...
        LOG_INFO_FMT("  Hop %d: SNR=%d dB (raw=0x%02X)", i + 1, snrDb, packet.path[i]);
    }
}
