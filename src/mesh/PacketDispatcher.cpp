...
void PacketDispatcher::addProcessor(IPacketProcessor* processor) {
    if (processor == nullptr || processorCount >= MAX_PROCESSORS) return;
    
    for (size_t i = 0; i < processorCount; ++i) {
        if (processors[i] == processor) return;
    }
    ...
    for (size_t i = 0; i < processorCount; ++i) {
        if (processors[i] != nullptr) {
            ...
            if (result == ProcessResult::DROP) {
                LOG_DEBUG_FMT("Packet dropped by processor: %s", processors[i]->getName());
                return;
            } else if (result == ProcessResult::STOP) {
                LOG_DEBUG_FMT("Pipeline stopped by processor: %s", processors[i]->getName());
                return;
            }
        }
    }
