...
bool PacketQueue::enqueue(const DecodedPacket& packet, int16_t rssi, int8_t snr, uint32_t timestamp) {
    if (isFull()) {
        droppedCount++;
        LOG_WARN_FMT("Packet queue full, dropping packet (total dropped: %lu)", droppedCount);
        return false;
    }
