...
void NodeConfig::initialize() {
    ...
    LOG_INFO_FMT("Node ID: 0x%04X, Hash: 0x%02X (from chip ID)", nodeId, nodeHash);
}

uint16_t NodeConfig::generateNodeIdFromChip() {
    uint8_t dieX = CY_GET_XTND_REG8((void *)CYREG_SFLASH_DIE_X);
    uint8_t dieY = CY_GET_XTND_REG8((void *)CYREG_SFLASH_DIE_Y);
    uint8_t dieWafer = CY_GET_XTND_REG8((void *)CYREG_SFLASH_DIE_WAFER);
    uint8_t dieLot0 = CY_GET_XTND_REG8((void *)CYREG_SFLASH_DIE_LOT0);
...
bool NodeConfig::loadFromEEPROM() {
    return false;
}
