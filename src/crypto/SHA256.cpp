#include "SHA256.h"

namespace MeshCrypto {

namespace {
constexpr uint32_t kInitState[8] = {
    0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a,
    0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19};

constexpr uint32_t kK[64] = {
    0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1,
    0x923f82a4, 0xab1c5ed5, 0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3,
    0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174, 0xe49b69c1, 0xefbe4786,
    0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
    0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147,
    0x06ca6351, 0x14292967, 0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13,
    0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85, 0xa2bfe8a1, 0xa81a664b,
    0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
    0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a,
    0x5b9cca4f, 0x682e6ff3, 0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208,
    0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2};

inline uint32_t rotr(uint32_t x, uint32_t n) {
  return (x >> n) | (x << (32 - n));
}

inline uint32_t ch(uint32_t x, uint32_t y, uint32_t z) {
  return (x & y) ^ (~x & z);
}

inline uint32_t maj(uint32_t x, uint32_t y, uint32_t z) {
  return (x & y) ^ (x & z) ^ (y & z);
}

inline uint32_t bigSigma0(uint32_t x) {
  return rotr(x, 2) ^ rotr(x, 13) ^ rotr(x, 22);
}

inline uint32_t bigSigma1(uint32_t x) {
  return rotr(x, 6) ^ rotr(x, 11) ^ rotr(x, 25);
}

inline uint32_t smallSigma0(uint32_t x) {
  return rotr(x, 7) ^ rotr(x, 18) ^ (x >> 3);
}

inline uint32_t smallSigma1(uint32_t x) {
  return rotr(x, 17) ^ rotr(x, 19) ^ (x >> 10);
}

} // namespace

SHA256::SHA256() { reset(); }

void SHA256::reset() {
  memcpy(ctx.state, kInitState, sizeof(kInitState));
  ctx.bitCount = 0;
  memset(ctx.buffer, 0, sizeof(ctx.buffer));
}

void SHA256::transform(const uint8_t block[64]) {
  uint32_t w[64];
  for (uint8_t i = 0; i < 16; ++i) {
    w[i] = (static_cast<uint32_t>(block[i * 4]) << 24) |
           (static_cast<uint32_t>(block[i * 4 + 1]) << 16) |
           (static_cast<uint32_t>(block[i * 4 + 2]) << 8) |
           (static_cast<uint32_t>(block[i * 4 + 3]));
  }
  for (uint8_t i = 16; i < 64; ++i) {
    w[i] = smallSigma1(w[i - 2]) + w[i - 7] + smallSigma0(w[i - 15]) +
           w[i - 16];
  }

  uint32_t a = ctx.state[0];
  uint32_t b = ctx.state[1];
  uint32_t c = ctx.state[2];
  uint32_t d = ctx.state[3];
  uint32_t e = ctx.state[4];
  uint32_t f = ctx.state[5];
  uint32_t g = ctx.state[6];
  uint32_t h = ctx.state[7];

  for (uint8_t i = 0; i < 64; ++i) {
    uint32_t t1 = h + bigSigma1(e) + ch(e, f, g) + kK[i] + w[i];
    uint32_t t2 = bigSigma0(a) + maj(a, b, c);
    h = g;
    g = f;
    f = e;
    e = d + t1;
    d = c;
    c = b;
    b = a;
    a = t1 + t2;
  }

  ctx.state[0] += a;
  ctx.state[1] += b;
  ctx.state[2] += c;
  ctx.state[3] += d;
  ctx.state[4] += e;
  ctx.state[5] += f;
  ctx.state[6] += g;
  ctx.state[7] += h;
}

void SHA256::update(const uint8_t *data, size_t len) {
  size_t bufferIndex = (ctx.bitCount / 8) % 64;
  ctx.bitCount += static_cast<uint64_t>(len) * 8;

  size_t partLen = 64 - bufferIndex;
  size_t dataIndex = 0;

  if (len >= partLen) {
    memcpy(&ctx.buffer[bufferIndex], data, partLen);
    transform(ctx.buffer);
    dataIndex = partLen;
    while (dataIndex + 63 < len) {
      transform(&data[dataIndex]);
      dataIndex += 64;
    }
    bufferIndex = 0;
  }

  memcpy(&ctx.buffer[bufferIndex], &data[dataIndex], len - dataIndex);
}

void SHA256::finalize(uint8_t digest[32]) {
  uint8_t padding[64] = {0x80};
  uint8_t lengthBytes[8];
  for (int i = 0; i < 8; ++i) {
    lengthBytes[7 - i] = static_cast<uint8_t>((ctx.bitCount >> (i * 8)) & 0xFF);
  }

  size_t bufferIndex = (ctx.bitCount / 8) % 64;
  size_t padLen = (bufferIndex < 56) ? (56 - bufferIndex) : (120 - bufferIndex);
  update(padding, padLen);
  update(lengthBytes, 8);

  for (uint8_t i = 0; i < 8; ++i) {
    digest[i * 4] = static_cast<uint8_t>(ctx.state[i] >> 24);
    digest[i * 4 + 1] = static_cast<uint8_t>(ctx.state[i] >> 16);
    digest[i * 4 + 2] = static_cast<uint8_t>(ctx.state[i] >> 8);
    digest[i * 4 + 3] = static_cast<uint8_t>(ctx.state[i]);
  }

  reset();
}

void SHA256::hmac(const uint8_t *key, size_t keyLen, const uint8_t *data,
                  size_t dataLen, uint8_t digest[32]) {
  uint8_t k0[64];
  memset(k0, 0, sizeof(k0));

  if (keyLen > 64) {
    SHA256 sha;
    sha.update(key, keyLen);
    sha.finalize(k0);
  } else {
    memcpy(k0, key, keyLen);
  }

  uint8_t innerPad[64];
  uint8_t outerPad[64];
  for (uint8_t i = 0; i < 64; ++i) {
    innerPad[i] = k0[i] ^ 0x36;
    outerPad[i] = k0[i] ^ 0x5C;
  }

  SHA256 shaInner;
  shaInner.update(innerPad, 64);
  shaInner.update(data, dataLen);
  uint8_t innerHash[32];
  shaInner.finalize(innerHash);

  SHA256 shaOuter;
  shaOuter.update(outerPad, 64);
  shaOuter.update(innerHash, 32);
  shaOuter.finalize(digest);
}

} // namespace MeshCrypto

