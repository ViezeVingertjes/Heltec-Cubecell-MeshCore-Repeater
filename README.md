...
- **Status Responder**: Responds to `!status` commands on public and private channels with node uptime status
...
**Note**: See [docs/HARDWARE_HTCC-AB02A.md](docs/HARDWARE_HTCC-AB02A.md) for detailed pin configuration and compatibility information.
...
- **`cubecell_board`**: Production build optimized for deployment (-O2, balanced size/performance)
- **`cubecell_board_debug`**: Debug build (-Og) with logging enabled for development
...
Configure private channel keys for secure communication:
...
The repeater will monitor all configured private channels and respond to `!status` commands on any of them.
...
namespace Power {
  ...
  // VEXT control is handled automatically by CubeCell framework
}
```
...
5. **PingResponder** (priority 35) - Responds to `!status` commands
6. **PacketLogger** (priority 99) - Logs packet details (debug mode only)
...
- VEXT power control handled by CubeCell framework
...
Send `!status` message to the public channel or private channel to query nearby repeaters. They will respond with:
- Node hash (last byte of Node ID)
- Wake time (time spent processing)
- Sleep time (time spent in low power mode)

**Supported Channels**:
- Public channel (default MeshCore public key)
- Private channels (configured in `Config.h`)

Repeaters automatically detect which channel the request came from and respond on the same channel. You can configure multiple private channels - the repeater will monitor all of them simultaneously.
...
**Production Build** (`cubecell_board`):
- `-O2`: Balanced optimization for size and performance
- `-fno-exceptions` / `-fno-rtti`: Remove C++ overhead (~3-7KB savings)
- `-ffast-math`: Fast floating-point operations (critical for low-latency forwarding)
- `-finline-functions`: Inline small functions
- `-fomit-frame-pointer`: Free up register for better performance
- `-fno-threadsafe-statics`: Remove unnecessary thread-safety overhead

**Debug Build** (`cubecell_board_debug`):
- `-Og`: Optimize for debugging experience
- `-g3`: Full debug symbols
- `-fno-inline`: Better stack traces
- `-DENABLE_LOGGING`: Enable serial logging

See `platformio.ini` for complete flag documentation.
...
- Built on the [Heltec CubeCell HTCC-AB02A platform](https://docs.heltec.cn/en/node/asr650x/htcc_ab02a/)
